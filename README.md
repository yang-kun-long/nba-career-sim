...
- 排行榜：基于当前赛季静态球员快照计算，可按得分、篮板、助攻、抢断、盖帽、三分和 TS% / eFG% 筛选，并支持最低出场数门槛与直接打开球员看板
- 高级统计：快照保存三分命中/出手、罚球出手、失误；TS% 与 eFG% 优先使用官方字段，缺失时根据基础投篮数据计算。位置仅在官方快照提供时作为筛选项显示
...
- 每日刷新：每天北京时间 02:00 拉取当前赛季快照；数据接口失败会做 3 轮延迟重试，将当前赛季合并到历史分片，重建球员/球队档案索引，校验后部署
